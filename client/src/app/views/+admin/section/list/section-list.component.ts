...
import { SectionAddComponent } from '../add/section-add.component';
...
const deleteManySections = gql`
  mutation deleteManySections($where: SectionWhereInput!) {
    deleteManySections(where: $where) {
      count
    }
  }
`;
...
@Component({
  ...
  template: `
    <div class="container">
      ...
      <div fxLayout="row" fxLayoutAlign="center center">
        <div class="item" fxFlex="98%">
          <button class="create-button" mat-raised-button color="primary" routerLink="/admin/section/create" routerLinkActive="active">
              ...
              <mat-table #table [dataSource]="dataSource" matSort aria-label="Elements">
                ...
                <ng-container matColumnDef="add">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button matTooltip="Adicionar ejercicio"
                ...
                <ng-container matColumnDef="details">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="accent" [routerLink]="['/admin','section', 'details', row.id]" routerLinkActive="active">
                ...
                <ng-container matColumnDef="edit">
                  ...
                  <mat-cell fxFlex="7" *matCellDef="let row">
                    <a mat-icon-button color="primary" [routerLink]="['/admin','section', 'update', row.id]" routerLinkActive="active">
...
export class SectionListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.querySubscription = this.apollo
      ...
      .valueChanges.subscribe(
        ({ data, loading }) => {
          ...
          if (data) {
            this.dataSource.data = data.sections;
          }
        },
        error => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(sections => sections.id !== section.id);
            this.snackBar.open(`Sección ${data.deleteSection.code} eliminado correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
          this.snackBar.open(error, 'X', {duration: 3000});
  ...
  onDeleteSelected($event): void {
    const dialogRef = this.dialog.open(ConfirmComponent, {
      data: {
        message: `¿Está seguro que desea eliminar los ${this.selection.selected.length} secciones selecionados?`,
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.apollo.mutate({
          ...
          if (data) {
            this.dataSource.data = this.dataSource.data.filter(section => !this._sectionExist(section, this.selection.selected));
            this.selection.clear();
            this.snackBar.open(`Los ${data.deleteManySections.count} secciones fueron eliminados correctamente`, 'X', {duration: 3000});
          }
        }, (error) => {
          this.loading = false;
          this.snackBar.open(error, 'X', {duration: 3000});
  ...
  private _sectionExist(section: Section, sections: any): boolean {
    if (sections.some(data => (data.id === section.id))) {
      return true;
    }
    return false;
  }
